...
// for Docker
if (__dirname.startsWith('/srv/nesty')) {
	assetPath = '/srv/nesty/apps/api/src/assets';
	assetPublicPath = '/srv/nesty/apps/api/public';
} else {
	assetPath = path.join(
		path.resolve(
			__dirname,
...
export const defaultConfiguration: IPluginConfig = {
	...
	plugins: [],
	authOptions: {
		expressSessionSecret: process.env.EXPRESS_SESSION_SECRET || 'nesty',
